...
SavvyCANServer::SavvyCANServer(HardwareSerial &console, WiFiServer &server) : console(console),
                                                                              wifiServer(server) {
...
void SavvyCANServer::handleWiFi(void) {
    if (settings.wifiMode > 0) {
        ...
        if (micros() - lastBroadcast > 1000000ul) //every second send out a broadcast ping
        {
            ...
            wifiUDPServer.beginPacket(broadcastAddr, SAVVYCAN_IP_PORT);
...
void SavvyCANServer::calculate_bus_load(void) {
    if (millis() > (busLoadTimer + 250)) {
        ...
        if (busLoad[1].busloadPercentage == 0 && busLoad[1].bitsSoFar > 0)
            busLoad[1].busloadPercentage = 1;
        busLoad[0].bitsPerQuarter = settings.CAN0Speed / 4;
        busLoad[1].bitsPerQuarter = settings.CAN1Speed / 4;
        busLoad[0].bitsSoFar = 0;
        busLoad[1].bitsSoFar = 0;
...
void SavvyCANServer::packFrameToBeSent(CAN_FRAME &frame, int whichBus) {
    ...
    if (SysSettings.lawicelMode) {
        ...
        if (settings.useBinarySerialComm) {
            ...
            if (frame.extended)
                sprintf((char *)&serialBuffer[serialBufferLength], " X ");
            else
                sprintf((char *)&serialBuffer[serialBufferLength], " S ");
